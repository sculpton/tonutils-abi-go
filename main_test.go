...
func TestGenerateTLBCustomPackUnpackAliasSupportsOneSidedDirections(t *testing.T) {
	...
	result, err := newGenerator([]abiFile{abi}, "sample").withStrict(true).Generate()
	src := result.Source
	if err != nil {
		t.Fatalf("generate: %v", err)
	}
...
func TestGenerateFixtureGenericTLBAliasesKeepCodecs(t *testing.T) {
	...
	result, err := newGenerator([]abiFile{abi}, "fixtures").Generate()
	src := result.Source
	if err != nil {
...
func TestGenerateWrapperFixtureABIs(t *testing.T) {
	...
	for _, fixture := range fixtures {
		...
		t.Run(name, func(t *testing.T) {
			result, err := GenerateFile([]string{fixture}, Options{PackageName: "fixtures"})
			src := result.Source
			if err != nil {
